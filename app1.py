...
# Movie selection dropdown (no sidebar)
movie_list = movies['title'].values
selected_movie = st.selectbox("Choose a movie from the list", movie_list)

# Number of recommendations slider (no sidebar)
num_recommendations = st.slider("Number of recommendations", 1, 10, 5)

# Show recommendations button
if st.button("Show Recommendations"):
    selected_movie_id = movies[movies['title'] == selected_movie].iloc[0].movie_id
    selected_movie_poster, selected_movie_title, selected_movie_overview, selected_movie_release_date, selected_movie_rating = fetch_movie_details(selected_movie_id)
    
    # Display selected movie info
    st.subheader(f"Selected Movie: {selected_movie_title} ({selected_movie_release_date})")
    col1, col2 = st.columns([1, 2])
    with col1:
        st.image(selected_movie_poster, use_column_width=True)
    with col2:
        st.markdown(f"**Rating**: {selected_movie_rating}/10")
